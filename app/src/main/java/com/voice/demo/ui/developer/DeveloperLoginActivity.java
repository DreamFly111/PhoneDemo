...
public class DeveloperLoginActivity extends LoginUIActivity implements View.OnClickListener {
	...
	private void showSubaccountDialog() {
		...
		builder.setTitle(R.string.str_select_voip_account).setItems(mVoipArray, new DialogInterface.OnClickListener() {
	...
	private void initConfigInfomation() {
		...
		if (CCPConfig.VoIP_ID_LIST != null) {
			...
			if (mVoipArray == null || mVoipArray.length == 0) {
				throw new IllegalArgumentException("Load the VOIP account information errors" + ", configuration information can not be empty" + mVoipArray);
			}
		}
	}
	/**
